...
<div class="container">
    ...
    <form action="passwordchange.do" method="post">
        ...
        <table class="table">
            ...
            <tbody>
                <c:forEach var="list" begin="1" end="24" varStatus="status">
                    <tr class="table-row">
                        <td><input type="checkbox" class="checkbox"></td>
                        <td class="group-id"></td>
                        <td class="group-name"><c:if test="${status.index <10}">0</c:if>${status.index}</td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
...
function choose() {
	...
	opener.document.getElementById("time").value = pgmgroupvalue;
	self.close();
}
