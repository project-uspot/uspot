...
<body>
	<div class="container">
		<h2>사물함코드 등록 및 변경</h2>
		<form action="lockercodeupdate.do" method="post">
			<div>
			...
			<div>
				...
				<input type="number" name="sortOrder" id="sortOrder" style="margin-top: 10px;" value="${sortorder}">
			</div>
			<div>
			...
			<div>
			 ...
			 <nav style="padding-right: 160px; margin-top: -10px;">
  				<input type="checkbox" id="isDeletecheck" name="isDeletecheck">
  				<input type="hidden" id="isDelete" name="isDelete">
  				<label for="isdelete">삭제</label>
  			</nav> 
  			<input type="hidden" name="pLockerID" value="${detail.PLockerID}">
			<button type="submit" style="margin-top: 20px;">저장</button>
		</form>
	</div>
	<script>
		document.getElementById("plockergroupname").value = ${detail.PLockerGroupID};
	 	document.getElementById("plockerno").value = ${detail.PLockerNO};
	  	document.getElementById("sortOrder").value = ${detail.sortOrder};
	  	document.getElementById("state").value = ${detail.state};
	 	document.getElementById("repairID").value = ${detail.repairID};
		document.getElementById("isDelete").value = '${detail.isDelete}';
		if(document.getElementById("isDelete").value==='Y'){
			$('#isDeletecheck').prop('checked', true);
		}
	  var PLockergroupid = document.getElementById("plockergroupname").value;
	  
	  $("#isDeletecheck").change(function(){
	        if(this.checked){
	        	document.getElementById("isDelete").value = 'Y';
	        }else{
	        	document.getElementById("isDelete").value = 'N';
	        }
	      });
